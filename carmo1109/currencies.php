    ...
    switch ($action) {
      ...
      case 'setstatus':
        ...
        if (isset($_POST['current_status']) && ($_POST['current_status'] == '0' || $_POST['current_status'] == '1')) {
          $db->Execute("UPDATE " . TABLE_CURRENCIES . " SET status='" . ($_POST['current_status'] == 0 ? 1 : 0) . "' WHERE currencies_id='" . (int)$currencies_id . "'");
          zen_redirect(zen_href_link(FILENAME_CURRENCIES, 'cID=' . (int)$currencies_id . '&page=' . $_GET['page']));
        }
        $action = '';
        break;
      case 'delete':
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="center"><?php echo TABLE_HEADING_CURRENCY_STATUS; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
<?php
  $currency_query_raw = "select currencies_id, title, code, symbol_left, symbol_right, decimal_point, thousands_point, decimal_places, last_updated, value, status from " . TABLE_CURRENCIES . " order by title";
...
?>
                ...
                <td class="dataTableContent" align="center">
                  <?php
                  echo zen_draw_form('setstatus', FILENAME_CURRENCIES, 'action=setstatus&cID=' . $currency->fields['currencies_id'] . '&page=' . $_GET['page']);
                  $formSRC = ($currency->fields['status'] == '0') ? 'icon_red_on.gif' : 'icon_green_on.gif';
                  $formTITLE = ($currency->fields['status'] == '0') ? IMAGE_ICON_STATUS_OFF : IMAGE_ICON_STATUS_ON;
                  ?>
                  <input type="image" src="<?php echo DIR_WS_IMAGES . $formSRC; ?>" alt="<?php echo $formTITLE; ?>" />
                  <input type="hidden" name="current_status" value="<?php echo $currency->fields['status']; ?>" />
                  </form>
                </td>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('text' => '<br>' . zen_draw_checkbox_field('status', '', 1) . ' ' . TABLE_HEADING_CURRENCY_STATUS);
    ...
    case 'edit':
      ...
      $contents[] = array('text' => '<br>' . zen_draw_checkbox_field('status', '', (isset($cInfo->status) ? $cInfo->status : 1)) . ' ' . TABLE_HEADING_CURRENCY_STATUS);
    ...
    default:
      if (is_object($cInfo)) {
        ...
        $contents[] = array('text' => '<br>' . TABLE_HEADING_CURRENCY_STATUS . ' ' . ($cInfo->status == 0 ? TEXT_NO : TEXT_YES));
